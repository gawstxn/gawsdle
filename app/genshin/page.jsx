...
export default function Home() {
  ...
  return (
    <div className="relative flex flex-col items-center mx-auto px-2">
      ...
      <p>Genshin Impact Update {game_verstion}</p>
        ...
        <div ref={domSearch} className="absolute max-h-[300px] bg-[#eee] text-black w-full border-x-2 border-[#eee] z-10 overflow-y-scroll">
          {search.map((data, index) => (
            <div className="flex items-center gap-4 p-2 border-b border-[#ccc] hover:bg-[#dadada] duration-150 cursor-pointer" key={index} onClick={(e) => changeSelectStage(index, e)}>
              <Image width={50} height={50} src={data.image} draggable={false}  alt="" className="w-10 h-10 object-cover"/>
              {data.name}
            </div>
          ))}
        </div>
      ...
      {/* head */}
      <div className="w-[350px] lg:w-[950px] grid grid-cols-6 text-center mt-8 bg-[#eee] border-x-2 text-black rounded-[14px_0_0_0] h-10 gap-[1px]">
        ...
        (data.select) ? (
          <div className="w-[350px] lg:w-[950px] grid grid-cols-6 text-center h-20 border-x-2 border-b-2 border-[#eee] gap-[1px] slide-in-bck-center scale-in-ver-top" key={index}>
            <div className="w-full h-full flex items-center justify-center relative px-2 my-auto mx-auto">
              <Image width={50} height={50} src={data.image} draggable={false} alt="" className="w-9 lg:w-12 h-9 lg:h-12 object-cover"/>
            ...
            <Image width={50} height={50} src={data.nation} draggable={false} alt="" className="brightness-125 w-8 lg:w-12 h-8 lg:h-12 object-cover"/>
            ...
            <div className="w-full h-full flex items-center justify-center relative px-2 my-auto mx-auto">
              <Image width={50} height={50} src={data.vision} draggable={false} alt="" className="w-8 lg:w-12 h-8 lg:h-12 object-cover"/>
            ...
            <div className="w-full h-full flex items-center justify-center relative px-2 my-auto mx-auto">
              <Image width={50} height={50} src={data.weapon} draggable={false} alt="" className="brightness-[1.7] w-9 lg:w-12 h-9 lg:h-12 object-cover"/>
        ...
        <div className="fixed w-full h-full top-0 bg-black bg-opacity-70 flex justify-center items-center z-10">
          <div className="fixed w-[350px] lg:w-[500px] border border-[#eee] bg-[#171717] rounded-[14px_0_0_0] overflow-hidden scale-in-center">
            ...
            <div className="flex flex-col justify-center items-center">
              <Image width={100} height={100} src={randomCharacter.image} draggable={false} alt="" className="mt-8 w-20 h-20 object-cover"/>
              <p className="mt-2 uppercase text-center">ANSWER IS {randomCharacter.name}</p>
              <p className="mt-2 uppercase">Guess: {guess}</p>
              <button onClick={playAgain} className="bg-[#eee] text-black mt-6 mb-4 p-2 rounded-[14px_0_0_0]">PLAY AGAIN</button>
            </div>
          </div>
        </div>
      ) : null }
